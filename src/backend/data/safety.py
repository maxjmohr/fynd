...
def create_country_safety_df():
    ...
    economics_and_peace_df.rename(columns={'Country':'country_2'}, inplace=True)

    # merge dataframes, use outer join to ensure countries for which not all scores are available will be included
    safety_df = pd.merge(safety_df, economics_and_peace_df[["iso3c", "peace_index", "terrorism_index", "ecological_threat", "country_2"]], 
                        left_on="ISO3", right_on="iso3c", how='outer')
    
    # unify duplicate columns by copying values and deleting superfluous columns
    safety_df.loc[safety_df['ISO3'].isnull(),'ISO3'] = safety_df.loc[safety_df['ISO3'].isnull(),'iso3c'].values
    safety_df.loc[safety_df['CountryName'].isnull(),'CountryName'] = safety_df.loc[safety_df['CountryName'].isnull(),'country_2'].values
    safety_df = safety_df.drop(["iso3c", "country_2"], axis=1)
