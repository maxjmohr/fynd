...
COMMENT ON COLUMN core_scores.ref_start_location_id IS 'Foreign key to the start location table. If filled, score is valid for the location and all its children';
...
COMMENT ON COLUMN core_scores.distance_to_median IS 'Distance to the median score of the dimension';
COMMENT ON COLUMN core_scores.distance_to_bound IS 'Distance to the upper or lower bound of the dimension. Positive if above, negative if below, NULLs if within bounds';
